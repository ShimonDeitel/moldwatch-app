import XCTest
@testable import Moldwatch

@MainActor
final class MoldwatchTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(MoldSpotEntry(title: "New One"))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(MoldSpotEntry(title: "ToDelete"))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        store.isPro = false
        store.entries = (0..<Store.freeLimit).map { MoldSpotEntry(title: "Item \($0)") }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(MoldSpotEntry(title: "Original"))
    ...
    func testDeleteAtOffsetsRemovesCorrectEntry() {
        store.entries = []
        store.add(MoldSpotEntry(title: "A"))
        store.add(MoldSpotEntry(title: "B"))
